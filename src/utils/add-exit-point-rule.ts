...
export function addExitPointRule (options: any): Rule {
    return (host: Tree) => {
        const navigationPath = options.config.parentExitPointComponent.path;
        ...
        const navBarElem = cheerioDom(`#${options.config.parentExitPointComponent.elementId}`);
        const contentToInsert = `<${options.config.parentExitPointComponent.componentSelector} label="${options.config.parentExitPointComponent.options.label}"></${options.config.parentExitPointComponent.componentSelector}>`;
